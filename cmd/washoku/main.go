...
func intent(q string) Intent {
	...
	for _, n := range []int{1000, 1500, 2000, 2500, 3000, 4000, 5000} {
		if strings.Contains(q, strconv.Itoa(n)) { i.BudgetMax = n; break }
	}
	if strings.Contains(q, "川崎") { i.Area = "川崎" }
...
func score(q string, r Restaurant) (float64, []string) {
	...
	if i.Instagram && has(r.Tags, "2000円") { s += 0.1 }
	return s, reasons
}

func normalize(s string) string {
	return strings.Map(func(r rune) rune { if unicode.IsSpace(r) { return -1 }; return unicode.ToLower(r) }, s)
}
...
func main() {
	...
	for _, r := range rs {
		...
		s, reasons := score(q, r)
		if s > 0 { results = append(results, Result{Restaurant: r, Score: s, Reason: reasons}) }
	}
	...
	for n, r := range results {
		...
		if len(r.Reason) > 0 { fmt.Printf("   → %s\n", strings.Join(r.Reason, "・")) }
	}
}
